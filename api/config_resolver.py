...
class ConfigResolver:
    ...
    def _resolve_brand_path(self, brand_id: str) -> Path:
        ...
        fallback_paths = sorted(
            path for path in self.config_root.glob("*.json") if path.is_file()
        )
    ...
    def resolve(self, manifest: GenerationManifest) -> ResolvedConfig:
        ...
        anthropic_cfg = _cfg("anthropic")
        people_cfg = _cfg("people")
        default_person = (
            people_cfg.get("default", {})
            if isinstance(people_cfg.get("default"), dict)
            else {}
        )
        export_defaults = _cfg("exportDefaults")
        ...
        manuscript_model = str(anthropic_cfg.get("manuscriptModel", default_model))
        media_model = str(anthropic_cfg.get("mediaModel", manuscript_model))
        ...
        return ResolvedConfig(
            ...
            script_prompt=str(
                prompts.get(
                    "scriptPrompt",
                    "Return JSON with key lines and 3-4 short factual lines"
