...
class AssetAnalysisService:
    ...
    def _describe_video_scenes(
        ...
        for idx, meta in enumerate(frame_meta):
            ...
            start_seconds = (
                float(start_raw) if isinstance(start_raw, (int, float)) else 0.0
            )
    ...
    def _describe_image_path(
        ...
        try:
            ...
            # Claude vision: strip the data URL prefix, keep raw base64 + media_type
                ...
                tools=[
                    {
                        ...
                        "description": "Output the image description",
                        "input_schema": DescriptionResult.model_json_schema(),
                    }
                ],
                tool_choice={"type": "tool", "name": "output_description"},
            )
            ...
            for block in response.content:
                if block.type == "tool_use":
                    ...
                    break
            if tool_input:
                parsed = DescriptionResult.model_validate(tool_input)
    ...
    def _assign_assets_to_script_lines(
        ...
        if not candidate_ids:
            ...
            model_ids = self._claude_assign_assets_to_script_lines(
                script_lines=script_lines,
            ...
            if not selected_ids:
                used_fallback = True
                selected_ids = self._fallback_asset_assignment(
                    script_lines, candidate_ids
                )
    ...
    def _claude_assign_assets_to_script_lines(
        self,
        script_lines: list[str],
        ...
        try:
            logger.info(
                "[asset-analysis] Requesting placement with model '%s' "
                "(lines=%d, candidates=%d)",
                model,
                len(script_lines),
                ...
                tools=[
                    {
                        ...
                        "description": "Output the asset placement for each line",
                        "input_schema": PlacementResult.model_json_schema(),
                    }
                ],
                tool_choice={"type": "tool", "name": "output_placement"},
            )
            tool_input: dict | None = None
            for block in response.content:
                if block.type == "tool_use":
            ...
            if tool_input is None:
                return []
            parsed = PlacementResult.model_validate(tool_input)
            selected_ids = [
                asset_id.strip()
            ...
            logger.warning("Placement with Claude failed: %s", exc)
            return []
    ...
    def _fallback_asset_assignment(
        ...
        if not candidate_ids:
            return []
        return [
            candidate_ids[idx % len(candidate_ids)] for idx in range(len(script_lines))
        ]
    ...
    def _predict_image_hotspots(
        ...
        logger.info(
            "Starting hotspot prediction for %d image assets", len(image_assets)
        )
        if not image_assets:
            payload = {
                "version": 1,
    ...
    def _run_hotspot_worker(
        ...
        try:
            ...
            if exit_code != 0:
                logger.warning(
                    "Hotspot worker exited with non-zero status: %d", exit_code
                )
                return {"provider": "noop", "results": {}}
