...
class PipelineService:
    ...
    def _asset_url(self, project_id: str, project_relative_path: str) -> str:
        base = self.settings.app_base_url
        return f"{base}/projects/{project_id}/files/{project_relative_path}"

    def _normalize_input_asset_path(
        self, input_folder: str, provided_path: str
    ) -> str:
    ...
    def _resolve_script_lines(
        ...
        is_str = isinstance(script_prompt_override, str)
        override_prompt = (
            script_prompt_override.strip()
            if is_str and script_prompt_override.strip()  # type: ignore[union-attr]
            else None
        )
    ...
    def _build_analysis_input_assets(
        ...
        for index, image in enumerate(article.images):
            rel_path = self._normalize_input_asset_path("images", image.path)
            local_path = self.store.resolve_asset_path(project_id, rel_path)
        ...
        for index, video in enumerate(article.videos):
            rel_path = self._normalize_input_asset_path("videos", video.path)
            local_path = self.store.resolve_asset_path(project_id, rel_path)
            analysis_input_assets.append(
                AnalysisInputAsset(
    ...
    def _map_analysis_asset_to_media_asset(
        ...
        fallback_url = self._asset_url(project_id, project_relative_path)
        asset_url = str(analysis_asset.get("url", fallback_url))
    ...
    def _build_global_media_assets(
        ...
        for analysis_asset in analysis_result.assets:
            ...
            has_scenes = isinstance(analysis_asset.get("videoScenes"), list)
            media_asset = self._map_analysis_asset_to_media_asset(
                project_id=project_id,
                analysis_asset=analysis_asset,
                video_scene_index=0 if has_scenes else None,
            )
    ...
    def _resolve_camera_movements(self, player_config: dict[str, Any]) -> list[str]:
        configured = player_config.get("defaultCameraMovements", [])
        if (
    ...
    def _build_segments_from_analysis(
        ...
        for index, script_line in enumerate(script_lines):
            ...
            if placed_asset_id and placed_asset_id in analysis_assets_by_id:
                placed = analysis_assets_by_id[placed_asset_id]
                video_scene_index: int | None = None
                video_scenes = placed.get("videoScenes")
                if isinstance(video_scenes, list) and len(video_scenes) > 0:
                    next_idx = next_video_scene_index_by_asset_id.get(
                        placed_asset_id, 0
                    )
                    video_scene_index = next_idx % len(video_scenes)
                    next_video_scene_index_by_asset_id[placed_asset_id] = (
                        next_idx + 1
                    )

                mapped = self._map_analysis_asset_to_media_asset(
                    project_id=project_id,
                    analysis_asset=placed,
                    video_scene_index=video_scene_index,
                )
                if mapped is not None:
                    selected_media_assets = [mapped]
            elif analysis_result.assets:
                fallback = analysis_result.assets[
                    index % len(analysis_result.assets)
                ]
                has_scenes = isinstance(fallback.get("videoScenes"), list)
                fallback_asset = self._map_analysis_asset_to_media_asset(
                    project_id=project_id,
                    analysis_asset=fallback,
                    video_scene_index=0 if has_scenes else None,
                )
                if fallback_asset is not None:
                    selected_media_assets = [fallback_asset]

            cam = default_camera_movements[index % len(default_camera_movements)]
            segments.append(
                Segment(
                    id=index + 1,
    ...
    def generate_manuscript(
        ...
        logger.info(
            "[pipeline:%s] Article loaded (images=%d, videos=%d, script=%s)",
            project_id,
            len(article.images),
            len(article.videos),
        ...
        logger.info(
            "[pipeline:%s] Step 4/7: Preparing analysis input assets", project_id
        )
        analysis_input_assets = self._build_analysis_input_assets(
            project_id=project_id, article=article
        )
        logger.info(
        ...
        logger.info(
            "[pipeline:%s] Asset analysis completed (assets=%d, hotspot=%s)",
            project_id,
        ...
        default_camera_movements = self._resolve_camera_movements(
        ...
        logger.info("[pipeline:%s] Step 7/7: Saved manuscript outputs", project_id)
    ...
    def _load_manuscript_for_processing(
        ...
        if manuscript is None:
            payload = self.store.load_json(project_id, "working/manuscript.json")
    ...
    def _render_segment_audio_timeline(
        ...
        logger.info(
            "[pipeline:%s] Audio rendering started "
            "(segments=%d, lines=%d, pause=%.2fs)",
            project_id,
        ...
        for segment_index, segment in enumerate(manuscript.segments, start=1):
            ...
            for text_line in segment.texts:
                ...
                logger.info(
                    "[pipeline:%s] Synthesizing line %d/%d (segment=%d, id=%d)",
                    project_id,
                    next_line_number,
                    total_line_count,
                    segment_index,
                ...
                clip_duration = self.tts_service.get_duration_seconds(
                    line_audio_clip_path
                )
                logger.info(
                ...
                text_line.end = round(current_timeline_seconds + clip_duration, 3)
                current_timeline_seconds += clip_duration
                ...
                if segment_pause_seconds > 0 and rendered_line_count < total_line_count:
                    if pause_audio_clip_path is None:
                        ...
                        self.tts_service.create_silence_mp3(
                            segment_pause_seconds, pause_audio_clip_path
                        )
            ...
            segment.end = round(
                max(
                    (text_line.end or segment_start_seconds)
                    for text_line in segment.texts
                ),
                3,
            )
            segment.text = "\n\n".join(
                text_line.displayText or text_line.text
                for text_line in segment.texts
            )
            logger.info(
    ...
    def process_manuscript(
        self, project_id: str, manuscript: Manuscript | None = None
    ) -> Manuscript:
        ...
        manuscript_to_process = self._load_manuscript_for_processing(
            project_id, manuscript
        )
        logger.info(
        ...
        logger.info(
            "[pipeline:%s] Step 4/4: Concatenating narration and saving output",
            project_id,
        )
        full_narration_path = (
            self.store.project_path(project_id) / "output" / "narration.mp3"
        )
        ...
        audio_rel = self.store.rel_to_project(project_id, full_narration_path)
        manuscript_to_process.meta.audio = {
            "src": self._asset_url(project_id, audio_rel)
        }

        output_payload = manuscript_to_process.model_dump(
            mode="json", exclude_none=True
        )
        output_payload["meta"]["processedAt"] = datetime.now(timezone.utc).isoformat()

        self.store.save_json(
            project_id, "output/processed_manuscript.json", output_payload
        )
